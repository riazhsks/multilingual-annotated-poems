...
def load_data(data_file):
    """Loads poem records, filters hold-out set, and prepares feature subsets."""
...
if __name__ == "__main__":
    ...
    for data in DATA:
        ...
        for sampling in ["standard", "downsampled"]:
            ...
            config_id = f"Cz-f-{sampling[0]}"
            
            # Map to the folder structure
            model_rel_path = f"full/best_model_{sampling}.joblib"
            ...
            # Apply and save
            df_res = apply_model(
                lang_label, config_id, 
                current_text_df, n_df, c_df, labels, 
    ...
    if all_results:
        ...
        master_df.to_csv("code/supervised/full_cz_models_preds.csv", index=False)